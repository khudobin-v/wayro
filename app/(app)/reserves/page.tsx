      ...
      <div className="px-4 py-4">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{RESERVE_TYPES.find((t) => t.key === reserve.type)?.label}</p>
          </div>
          <div className="text-right">
            <p className={`text-lg font-bold tabular-nums ${critical ? 'text-danger' : 'text-gray-900'}`}>
              {formatMoney(reserve.balance)}
            </p>
            {reserve.goal && (
              <p className="text-xs text-gray-400">из {formatMoney(reserve.goal)}</p>
            )}
          </div>
        </div>
        ...
        {pct !== null && (
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-400 mt-1">{Math.round(pct)}% накоплено</p>
          </div>
        )}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 glass border border-gray-200 rounded-xl py-2 text-xs text-gray-400 hover:text-gray-900 flex items-center justify-center gap-1.5"
          >
            <Minus size={12} /> Списать
          </button>
          <button
            onClick={() => setExpanded((v) => !v)}
            className="px-3 glass border border-gray-200 rounded-xl py-2 text-xs text-gray-400 hover:text-gray-900 flex items-center gap-1"
          >
        ...
        <div className="border-t border-gray-200 px-4 py-3 space-y-2">
          {reserve.entries.map((e) => (
            <div key={e.id} className="flex items-center justify-between text-xs">
              <div>
                <span className={e.type === 'accrual' ? 'text-gray-900' : 'text-danger'}>
                  {e.type === 'accrual' ? '+' : '−'}{formatMoney(e.amount)}
                </span>
                {e.comment && <span className="text-gray-400 ml-2">{e.comment}</span>}
              </div>
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      {!isLoading && reserves?.length === 0 && (
        <div className="text-center py-16">
          <p className="text-gray-400 mb-2 text-sm">Резервов нет</p>
          <p className="text-gray-300 text-xs mb-5">Создайте резервы для ТО, налогов, шин</p>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Новый резерв">
        <form onSubmit={createReserve} className="px-5 py-4 space-y-3">
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Тип</label>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Цель накопления (₽)</label>
          ...
          {newReserve.type === 'maintenance' && (
            <>
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Стоимость ТО (₽)</label>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Интервал (км)</label>
          ...
          {newReserve.type === 'tax' && (
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">Процент от заработка</label>
          ...
          {(['tires', 'insurance', 'custom'] as const).includes(newReserve.type as 'tires' | 'insurance' | 'custom') && (
            <>
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Сумма события (₽)</label>
                <input type="number" inputMode="decimal" placeholder="0" value={newReserve.costPerEvent} onChange={(e) => setNewReserve((f) => ({ ...f, costPerEvent: e.target.value }))} className={inp} />
              </div>
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Интервал (месяцев)</label>
      ...
      <Modal open={!!withdrawTarget} onClose={() => { setWithdrawTarget(null); setWithdrawAmount(''); setWithdrawComment('') }} title={`Списание: ${withdrawTarget?.name}`}>
        <form onSubmit={handleWithdraw} className="px-5 py-4 space-y-3">
          <div className="glass rounded-xl px-4 py-3 text-center">
            ...
            <p className="text-xl font-bold text-gray-900">{withdrawTarget ? formatMoney(withdrawTarget.balance) : '—'}</p>
          </div>
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Сумма списания (₽) *</label>
          ...
          <button type="submit" disabled={loading} className="w-full bg-danger text-gray-900 font-bold rounded-2xl py-3.5 text-sm disabled:opacity-60">
